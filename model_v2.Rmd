...
budget_suggestion <- function(input_data){
  ...
  max_increase <- 1000000000  # Maximum budget increase
  ...
  while (next_budget <= current_budget + max_increase) {
    ...
    if (as.character(new_bin) == next_bin) {
      # suggestion <- paste("By increasing your budget by", next_budget - current_budget, "your gross revenue will sit in", next_bin)
      return(paste("By increasing your budget by", next_budget - current_budget, "your gross revenue will sit in the", next_bin, "gross revenue bin. Your total budget would be", next_budget,"."))
    }
  }
...
saveRDS(budget_suggestion, file = "budget_suggestion.rds")
...
runtime_suggestion <- function(input_data){
  ...
  if (current_index < length(labels)) {
    ...
    return(paste("No higher bin")) 
  }

  next_runtime <- current_runtime
  max_increase <- 240  # Maximum runtime increase
  
  while (next_runtime <= current_runtime + max_increase) {
    next_runtime <- next_runtime + 1
    input_data$runtime <- next_runtime
    new_pred <- predict(rf_model, newdata = input_data)
    new_bin <- cut(new_pred, breaks = bins, labels = labels, include.lowest = TRUE)
    
    if (as.character(new_bin) == next_bin) {
      # suggestion <- paste("By increasing your runtime by", next_runtime - current_runtime, "your gross revenue will sit in", next_bin)
...
saveRDS(runtime_suggestion, file = "runtime_suggestion.rds")
