...
# Assuming numeric_vars is your dataframe with numeric variables
correlation_matrix <- cor(numeric_vars)
...
# Ensure you have the final_movieset dataset with the same structure as above
last_movie <- final_movieset %>% select(name, genre, budget, gross, rating, runtime)
print(last_movie)

# One-hot encode the genre and rating columns in last_movie
...
train_indices <- sample(1:nrow(last_movie), 0.8 * nrow(last_movie))  # 80% for training
...
## Plotting Graphs For Genre vs Gross Revenue
```{r}
# List of genres to plot
final_movieset <- final_movieset %>%
  mutate(normal_binned = cut(gross, breaks = bins, labels = labels, right = FALSE))

genres <- c("Mystery", "Horror", "Fantasy", "Drama", "Crime", "Comedy", "Biography", "Animation", "Adventure", "Action")

# Function to create histograms for a given genre
plot_genre <- function(genre) {
  genre_data <- final_movieset %>% filter(genre == !!genre)
  ggplot(genre_data, aes(x = normal_binned)) +
    geom_bar() +
...
# Create scatter plots for each genre and print them
for (genre in genres) {
  print(plot_genre(genre))
}
