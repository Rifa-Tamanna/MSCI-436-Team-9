...
```
For the analysis, we will be using year, score, log(votes), log(budget) and runtime. They all have reasonably broad distributions which make them appropriate candid ates.
...
final_movieset <- movieset_clean %>% select(rating, genre, year, score, votes, budget, gross, runtime)
...
```{r}
full_model <- lm(gross~rating+genre+year+score+votes+budget+runtime, data=train)
summary(full_model)
```
## Creating a model 
```{r}
...
```{r}
fit_model <- lm(gross~rating+genre+year+score+budget+runtime, data=train)
print(fit_model)
```
...
```{r}
fit_model_2 <- lm(gross~rating+genre+year+score+runtime, data=train)
print(fit_model_2)
```
...
```{r}
fit_model_3 <- lm(gross~rating+year+score+runtime, data=train)
print(fit_model_3)
```

```{r}
fit4 <- lm(gross ~ votes + budget + genre + rating, data=train)
summary(fit4)
```

```{r}
fit5 <- lm(gross ~ votes + budget + genre + rating + score, data=train)
summary(fit5)
```

```{r}
fit6 <- lm(gross ~ votes + budget + genre + rating + score + runtime, data=train)
summary(fit5)
```

## Predicting the Model
```{r}
new_movie <- test %>% select(votes, budget, genre, rating, score, runtime)
