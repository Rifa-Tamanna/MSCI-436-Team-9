...
final_movieset <- movieset_clean %>% select(name, rating, genre, year, score, votes, country, budget, gross, runtime)
...
```{r}
full_model <- lm(gross~rating+genre+year+score+votes+country+budget+runtime, data=train)
summary(full_model)
```
